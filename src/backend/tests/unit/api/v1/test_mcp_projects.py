...
from all-ai.api.v1.mcp_projects import (
...
from all-ai.services.auth.utils import create_user_longterm_token, get_password_hash
from all-ai.services.database.models.flow import Flow
from all-ai.services.database.models.folder import Folder
...
def mock_project_mcp_server():
    with patch("all-ai.api.v1.mcp_projects.ProjectMCPServer") as mock:
...
def mock_sse_transport():
    with patch("all-ai.api.v1.mcp_projects.SseServerTransport") as mock:
...
def mock_current_user_ctx(active_user):
    with patch("all-ai.api.v1.mcp_projects.current_user_ctx") as mock:
...
def mock_current_project_ctx(mock_project):
    with patch("all-ai.api.v1.mcp_projects.current_project_ctx") as mock:
...
def disable_mcp_composer_by_default():
    ...
    with patch("all-ai.api.v1.mcp_projects.get_settings_service") as mock_get_settings:
...
async def test_project_sse_creation(user_test_project):
    ...
    assert mcp_server.server.name == f"all-ai-mcp-project-{project_id}"
...
async def test_init_mcp_servers_error_handling():
    ...
    # Apply the patch
    with patch("all-ai.api.v1.mcp_projects.get_project_sse", side_effect=mock_get_project_sse):
