...
from all-ai.services.database.models.user import User
...
@pytest.fixture
def mock_mcp_server():
    with patch("all-ai.api.v1.mcp.server") as mock:
...
def mock_sse_transport():
    with patch("all-ai.api.v1.mcp.sse") as mock:
...
def mock_current_user_ctx(mock_user):
    with patch("all-ai.api.v1.mcp.current_user_ctx") as mock:
