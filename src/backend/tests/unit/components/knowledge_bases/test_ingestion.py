...
import pytest
from all-ai.base.knowledge_bases.knowledge_base_utils import get_knowledge_bases
from all-ai.components.knowledge_bases.ingestion import KnowledgeIngestionComponent
...
class TestKnowledgeIngestionComponent(ComponentTestBaseWithClient):
    ...
    def mock_knowledge_base_path(self, tmp_path):
        ...
        with patch("all-ai.components.knowledge_bases.ingestion.KNOWLEDGE_BASES_ROOT_PATH", tmp_path):
            yield
    ...
    @patch("all-ai.components.knowledge_bases.ingestion.get_settings_service")
    @patch("all-ai.components.knowledge_bases.ingestion.encrypt_api_key")
    ...
    async def test_convert_df_to_data_objects(self, component_class, default_kwargs):
        ...
        with patch("all-ai.components.knowledge_bases.ingestion.Chroma") as mock_chroma:
    ...
    async def test_convert_df_to_data_objects_no_duplicates(self, component_class, default_kwargs):
        ...
        with patch("all-ai.components.knowledge_bases.ingestion.Chroma") as mock_chroma:
            ...
            with patch("all-ai.components.knowledge_bases.ingestion.hashlib.sha256") as mock_hash:
    ...
    @patch("all-ai.components.knowledge_bases.ingestion.json.loads")
    @patch("all-ai.components.knowledge_bases.ingestion.decrypt_api_key")
