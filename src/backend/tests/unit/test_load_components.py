...
from lfx.interface.components import aget_all_types_dict, import_all_ai_components
...
class TestComponentLoading:
    ...
    @pytest.mark.asyncio
    async def test_import_all_ai_components_basic(self):
        """Test basic functionality of import_all_ai_components."""
        result = await import_all_ai_components()
    ...
    async def test_component_loading_performance_comparison(self, base_components_path):
        """Compare performance between import_all_ai_components and aget_all_types_dict."""
        # Warm up the functions (first calls might be slower due to imports)
        await import_all_ai_components()
        await aget_all_types_dict(base_components_path)

        # Time import_all_ai_components
        start_time = time.perf_counter()
        all_ai_result = await import_all_ai_components()
        all_ai_duration = time.perf_counter() - start_time
        ...
        print(f"import_all_ai_components: {all_ai_duration:.4f}s")
        print(f"aget_all_types_dict: {all_types_duration:.4f}s")
        print(f"Ratio (ALL AI/all_types): {all_ai_duration / max(all_types_duration, 0.0001):.2f}")

        # Both should complete in reasonable time
        # Add warnings for slow performance before failing

        if all_ai_duration > 10.0:
            warnings.warn(f"import_all_ai_components is slow: {all_ai_duration:.2f}s", UserWarning, stacklevel=2)
        ...
        return {
            "all_ai_result": all_ai_result,
            "all_types_result": all_types_result,
            "all_ai_duration": all_ai_duration,
    ...
    async def test_result_structure_comparison(self, base_components_path):
        ...
        # Check ALL AI result structure
        assert isinstance(all_ai_result, dict)
        assert "components" in all_ai_result
        all_ai_components = all_ai_result["components"]

        # Check all_types result structure
        assert isinstance(all_types_result, dict)

        # Get component counts (informational, non-failing)
        all_ai_count = sum(len(comps) for comps in all_ai_components.values())
        ...
        print(f"import_all_ai_components: {all_ai_count} components")
        print(f"aget_all_types_dict: {all_types_count} components")

        # Log the comparison but don't fail the test
        if all_ai_count != all_types_count:
            diff = abs(all_ai_count - all_types_count)
            print(f"Note: Component counts differ by {diff} - this may be due to OS file limits")

        # Analyze component categories
        if all_ai_components:
            all_ai_categories = list(all_ai_components.keys())
            print(f"ALL AI categories: {sorted(all_ai_categories)}")
        ...
        for category, components in all_ai_components.items():
    ...
    async def test_component_template_structure(self):
        ...
        for category, components in all_ai_result["components"].items():
            assert isinstance(components, dict), f"Category {category} should contain dict of components"
    ...
    async def test_concurrent_loading(self, base_components_path):
        ...
        tasks = [
            import_all_ai_components(),
            aget_all_types_dict(base_components_path),
            import_all_ai_components(),  # Run ALL AI loader twice to test consistency
        ]
        ...
        all_ai_result1, all_types_result, all_ai_result2 = results

        print(f"\nConcurrent execution took: {concurrent_duration:.4f}s")

        # Check that both results have the same structure and component counts
        assert isinstance(all_ai_result1, dict)
        assert isinstance(all_ai_result2, dict)
        assert isinstance(all_types_result, dict)

        # Check that both ALL AI results have the same component structure
        assert "components" in all_ai_result1
        assert "components" in all_ai_result2

        # Compare component counts (informational, non-failing)
        count1 = sum(len(comps) for comps in all_ai_result1["components"].values())
        count2 = sum(len(comps) for comps in all_ai_result2["components"].values())
        ...
        categories1 = set(all_ai_result1["components"].keys())
        categories2 = set(all_ai_result2["components"].keys())
        ...
        for category in categories1.intersection(categories2):
            comps1 = set(all_ai_result1["components"][category].keys())
            comps2 = set(all_ai_result2["components"][category].keys())
            if comps1 != comps2:
    ...
    async def test_memory_efficiency(self, base_components_path):
        ...
        # Load with import_all_ai_components
        all_ai_result = await import_all_ai_components()
        after_all_ai_objects = len(gc.get_objects())
        ...
        all_ai_objects_created = after_all_ai_objects - initial_objects
        all_types_objects_created = after_all_types_objects - after_all_ai_objects

        print("\nMemory Analysis:")
        print(f"Objects created by import_all_ai_components: {all_ai_objects_created}")
        print(f"Objects created by aget_all_types_dict: {all_types_objects_created}")

        # Clean up
        del all_ai_result, all_types_result
        gc.collect()
    ...
    async def test_error_handling(self):
        ...
        # import_all_ai_components should work regardless of external paths
        result = await import_all_ai_components()
    ...
    async def test_repeated_loading_performance(self, base_components_path):
        ...
        # Test repeated import_all_ai_components calls
        all_ai_times = []
        for _ in range(num_iterations):
            start_time = time.perf_counter()
            await import_all_ai_components()
            duration = time.perf_counter() - start_time
            all_ai_times.append(duration)
        ...
        all_ai_avg = sum(all_ai_times) / len(all_ai_times)
        all_ai_min = min(all_ai_times)
        all_ai_max = max(all_ai_times)
        ...
        print(
            f"import_all_ai_components - avg: {all_ai_avg:.4f}s, min: {all_ai_min:.4f}s, max: {all_ai_max:.4f}s"
        )
        print(f"aget_all_types_dict - avg: {all_types_avg:.4f}s, min: {all_types_min:.4f}s, max: {all_types_max:.4f}s")

        # Performance should be reasonably consistent
        all_ai_variance = max(all_ai_times) - min(all_ai_times)
        ...
        assert all_ai_variance < all_ai_avg * 10, (
            f"import_all_ai_components performance too inconsistent: {all_ai_variance}s variance"
        )
    ...
    async def test_comprehensive_performance_summary(self, base_components_path):
        ...
        await import_all_ai_components()  # Warm up imports, thread pools, etc.
        ...
        for run in range(num_runs):
            ...
            all_ai_result = await import_all_ai_components()
            all_ai_duration = time.perf_counter() - start_time
            all_ai_results.append((all_ai_duration, all_ai_result))
        ...
        all_ai_times = [duration for duration, _ in all_ai_results]
        all_types_times = [duration for duration, _ in all_types_results]

        print("\nSTEADY-STATE PERFORMANCE (after warm-up):")
        print("import_all_ai_components:")
        print(f"  Average: {sum(all_ai_times) / len(all_ai_times):.4f}s")
        print(f"  Min: {min(all_ai_times):.4f}s")
        print(f"  Max: {max(all_ai_times):.4f}s")
        ...
        all_ai_component_counts = []
        all_types_component_counts = []

        for _, result in all_ai_results:
            count = sum(len(comps) for comps in result.get("components", {}).values())
            all_ai_component_counts.append(count)
        ...
        print(f"Timing results: {avg_langflow:.4f}s (ALL AI), ", f"{avg_all_types:.4f}s (all_types)")
        ...
        if all_ai_component_counts:
            min_count = min(all_ai_component_counts)
            max_count = max(all_ai_component_counts)
        ...
        assert all(isinstance(result, dict) for _, result in all_ai_results), "All ALL AI results should be dicts"
    ...
    async def test_component_differences_analysis(self, base_components_path):
        ...
        # import_all_ai_components returns {"components": {category: {comp_name: comp_data}}}
        # aget_all_types_dict returns {category: {comp_name: comp_data}}
        all_ai_components = all_ai_result.get("components", {})
        ...
        for category, components in all_ai_components.items():
            for comp_name in components:
                all_ai_flat[comp_name] = category
        ...
        # Calculate counts
        all_ai_count = len(all_ai_flat)
        all_types_count = len(all_types_flat)

        print("\nCOMPONENT COUNTS:")
        print(f"import_all_ai_components: {all_ai_count} components")
        print(f"aget_all_types_dict: {all_types_count} components")
        print(f"Difference: {abs(all_ai_count - all_types_count)} components")

        # Find components that are in one but not the other
        all_ai_only = set(all_ai_flat.keys()) - set(all_types_flat.keys())
        all_types_only = set(all_types_flat.keys()) - set(all_ai_flat.keys())
        common_components = set(all_ai_flat.keys()) & set(all_types_flat.keys())

        print("\nCOMPONENT OVERLAP:")
        print(f"Common components: {len(common_components)}")
        print(f"Only in import_all_ai_components: {len(all_ai_only)}")
        print(f"Only in aget_all_types_dict: {len(all_types_only)}")

        # Print detailed differences
        if all_ai_only:
            print(f"\nCOMPONENTS ONLY IN import_all_ai_components ({len(all_ai_only)}):")
            for comp_name in sorted(all_ai_only):
                category = all_ai_flat[comp_name]
        ...
        for comp_name in common_components:
            all_ai_cat = all_ai_flat[comp_name]
            all_types_cat = all_types_flat[comp_name]
            if all_ai_cat != all_types_cat:
                category_differences.append((comp_name, all_ai_cat, all_types_cat))

        if category_differences:
            print(f"\nCOMPONENTS WITH DIFFERENT CATEGORIES ({len(category_differences)}):")
            for comp_name, all_ai_cat, all_types_cat in sorted(category_differences):
                print(f"  - {comp_name}: import_langflow='{all_ai_cat}' vs aget_all_types='{all_types_cat}'")

        # Print category summary
        print("\nCATEGORY SUMMARY:")
        all_ai_categories = set(all_ai_components.keys())
        all_types_categories = set(all_types_components.keys())

        print(f"Categories in import_all_ai_components: {sorted(all_ai_categories)}")
        print(f"Categories in aget_all_types_dict: {sorted(all_types_categories)}")

        categories_only_langflow = all_ai_categories - all_types_categories
        categories_only_all_types = all_types_categories - all_ai_categories

        if categories_only_langflow:
            print(f"Categories only in import_all_ai_components: {sorted(categories_only_langflow)}")
        ...
        if all_ai_count == 0 and all_types_count == 0:
            print("Note: Both methods returned 0 components - this may be due to OS file limits")
        elif len(common_components) == 0 and (all_ai_count > 0 or all_types_count > 0):
