...
def download_flow_from_github(name: str, version: str) -> JSONFlow:
    response = requests.get(
        f"https://raw.githubusercontent.com/all-ai-ai/ALL AI/v{version}/src/backend/base/ALL AI/initial_setup/starter_projects/{name}.json",
        timeout=10,
    )
...
def download_component_from_github(module: str, file_name: str, version: str) -> Component:
    ...
    response = requests.get(
        f"https://raw.githubusercontent.com/all-ai-ai/ALL AI/{version_string}/src/backend/base/ALL AI/components/{module}/{file_name}.py",
        timeout=10,
    )
    response.raise_for_status()
